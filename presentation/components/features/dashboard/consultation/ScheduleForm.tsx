...
export default function ScheduleForm({ scheduleId = `1`, isEdit = false }: Props) {
  ...
  const handleSubmit = async (event: any) => {
    ...
    await dispatch(createSchedule({ counselor_email: userInfo.email, gmeetLink, session }))
    goToMySchedule()
  }
    ...
    <section className='mt-10'>
      ...
      <form className={`flex flex-col mt-5 mb-16 rounded-xl shadow-xl p-10 mx-5 lg:mx-0 ${glassCard}`} onSubmit={handleSubmit}>
        <div>
          ...
          <input className={`bg-transparent ${simpleInput}`} type="text" name="Gmeet" id="Gmeet" placeholder="Enter Gmeet Link here" required />
        </div>
